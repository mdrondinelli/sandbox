...
Visibility_buffer::Visibility_buffer(
    ...
    : _extents{create_info.extents},
      ...
      _motion_vectors_texture{wrappers::make_unique_texture(GL_TEXTURE_2D)},
  ...
  glTextureStorage2D(_normal_texture.get(),
                     ...
                     create_info.extents.x,
                     create_info.extents.y);
  glTextureStorage2D(_motion_vectors_texture.get(),
                     1,
                     GL_RG16_SNORM,
                     create_info.extents.x,
                     create_info.extents.y);
  ...
  glTextureParameteri(
      _motion_vectors_texture.get(), GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  glTextureParameteri(
      _motion_vectors_texture.get(), GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  ...
  glNamedFramebufferTexture(_framebuffer.get(),
                            GL_COLOR_ATTACHMENT2,
                            _motion_vectors_texture.get(),
                            0);
  auto const draw_buffers = std::array<GLenum, 3>{
