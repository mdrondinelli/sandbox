...
float calculate_csm_shadow_factor(vec3 p_world, float z_view, float n_dot_l) {
  ...
  vec3 bias = (0.1 + length(random_offset)) * csm.cascades[i].pixel_length * tan(acos(n_dot_l)) * directional_light_direction.xyz;
...
void main() {
  ...
  for (int i = 0; i < 8; ++i) {
    vec3 value = texture(sample_buffer, texcoord + neighborhood_sample_offsets[i] / textureSize(sample_buffer, 0)).rgb;
...
void Render_stream::render() {
  ...
  auto const taa_blend_factor = [&]{
    if (_frame_time) {
      ...
      return 1.0f - pow(0.01f, frame_duration.count());
    } else {
      return 1.0f;
    }
  }();
