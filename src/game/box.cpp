...
math::Vec3f Box::get_half_extents() const noexcept {
  return std::visit(
      [&](auto const body) -> math::Vec3f {
        ...
        if constexpr (std::is_same_v<T, physics::Rigid_body> ||
                      std::is_same_v<T, physics::Static_body>) {
...
void Box::set_rigid_body_motion_callback(
    physics::Rigid_body_motion_callback *callback) noexcept {
  _world->data(std::get<physics::Rigid_body>(_body.specific()))
...
Box_manager::Box_manager(Box_manager_create_info const &create_info)
  ...
  _memory = util::assign_merged(util::System_allocator{},
                                std::tie(_boxes),
                                std::tuple{create_info.max_boxes});
}
...
Box *Box_manager::create_box(Rigid_box_create_info const &create_info) {
  ...
  auto const body = _world->create_rigid_body({
      ...
      .inertia_tensor =
          create_info.mass * (create_info.hollow
                                  ? physics::surface_inertia_tensor(shape)
...
void Box_manager::set_box_creation_callback(
    Box_creation_callback *callback) noexcept {
  _box_creation_callback = callback;
}

void Box_manager::set_box_destruction_callback(
    Box_destruction_callback *callback) noexcept {
