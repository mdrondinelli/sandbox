...
public:
  static constexpr util::Size memory_requirement(util::Size max_particles,
                                                 util::Size max_rigid_bodies,
                                                 util::Size max_static_bodies) {
    return util::memory_requirement<Allocator,
                                    decltype(_particles),
                                    decltype(_rigid_bodies),
                                    decltype(_static_bodies)>(
  ...
  make(Allocator &&allocator,
    ...
    auto const block = allocator.alloc(
        memory_requirement(max_particles, max_rigid_bodies, max_static_bodies));
    return {block,
            Object_storage{
                block, max_particles, max_rigid_bodies, max_static_bodies}};
  }
  ...
  template <typename F> void for_each_particle(F &&f) {
    _particles.for_each(f);
  }

  template <typename F> void for_each_rigid_body(F &&f) {
  ...
  void destroy(Particle object) {
    return _particles.destroy(object);
  }

  void destroy(Rigid_body object) {
    return _rigid_bodies.destroy(object);
  }

  void destroy(Static_body object) {
    return _static_bodies.destroy(object);
  }
