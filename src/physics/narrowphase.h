...
particle_shape_contact(float particle_radius,
  ...
  if (distance2 <= contact_distance2) {
    ...
    return Narrowphase_result{normal,
                              {particle_local_position, ball_local_position},
...
inline std::optional<Narrowphase_result> particle_shape_contact(
...
particle_shape_contact(float particle_radius,
  ...
  if (distance_squared > particle_radius_squared) {
    ...
    return Narrowphase_result{
        normal,
        {box_local_clamped_particle_position, particle_local_position},
        separation};
  } else {
    ...
    return Narrowphase_result{
        normal,
        {Vec3f::zero(), box_local_clamped_particle_position},
        separation};
  }
}
...
shape_shape_contact(Ball const &b1,
  ...
  if (distance_squared > contact_distance_squared) {
    ...
    return Narrowphase_result{normal,
                              {b1_local_position, b2_local_position},
                              distance - contact_distance};
  } else {
    return Narrowphase_result{
        Vec3f::x_axis(), {Vec3f::zero(), Vec3f::zero()}, -contact_distance};
  }
}
...
shape_shape_contact(Ball const &ball,
  ...
  if (distance2 > ball.radius * ball.radius) {
    ...
    return Narrowphase_result{
        normal,
        {ball_local_position, box_local_clamped_ball_position},
    ...
    return Narrowphase_result{normal,
                              {Vec3f::zero(), box_local_ball_position},
...
shape_shape_contact(Box const &b1,
  ...
  if (best_separating_axis_index < 3) {
    ...
    return Narrowphase_result{
        normal, {b1_local_position, b2_local_position}, best_separation};
...
object_object_contact(Particle_data const &first,
  ...
  if (distance_squared < contact_distance_squared) {
    ...
    return Narrowphase_result{
        normal,
        {position - first.position(), position - second.position()},
        separation};
  } else {
    return std::nullopt;
  }
