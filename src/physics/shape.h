...
public:
  ...
  friend std::optional<Narrowphase_result> particle_shape_contact(float particle_radius,
                                                                  math::Vec3f const &particle_position,
                                                                  Shape const &shape,
                                                                  math::Mat3x4f const &shape_transform,
                                                                  math::Mat3x4f const &shape_transform_inv) noexcept;

  friend std::optional<Narrowphase_result> shape_shape_contact(Shape const &shape_a,
...
inline math::Aabb3f bounds(Ball const &ball, math::Vec3f const &position) {
  return {position - math::Vec3f::all(ball.radius), position + math::Vec3f::all(ball.radius)};
}

inline math::Aabb3f bounds(Capsule const &capsule, math::Vec3f const &position, math::Vec3f const &axis) noexcept {
  auto const world_space_half_extents = abs(axis) + math::Vec3f::all(capsule.radius);
  return {position - world_space_half_extents, position + world_space_half_extents};
}

inline math::Aabb3f bounds(Box const &box, math::Mat3x4f const &transform) noexcept {
  auto const world_space_center = column(transform, 3);
  auto const world_space_half_extents = abs(transform) * math::Vec4f{box.half_extents, 0.0f};
  return math::Aabb{world_space_center - world_space_half_extents, world_space_center + world_space_half_extents};
}

inline math::Aabb3f bounds(Shape const &shape, math::Mat3x4f const &transform) noexcept {
  return std::visit(
      [&](auto &&arg) {
...
constexpr math::Mat3x3f solid_inertia_tensor(Ball const &ball) noexcept {
  ...
  return 2.0f / 5.0f * math::Mat3x3f{{r2, 0.0f, 0.0f}, {0.0f, r2, 0.0f}, {0.0f, 0.0f, r2}};
}
...
constexpr math::Mat3x3f solid_inertia_tensor(Capsule const &capsule) noexcept {
  ...
  auto const numer_perpendicular = 20.0f * h3 + 40.0f * h2 * r + 45.0f * h * r2 + 16.0f * r3;
  ...
  return math::Mat3x3f{{perpendicular, 0.0f, 0.0f}, {0.0f, parallel, 0.0f}, {0.0f, 0.0f, perpendicular}};
}
...
constexpr math::Mat3x3f solid_inertia_tensor(Box const &box) noexcept {
  ...
  return 1.0f / 3.0f * math::Mat3x3f{{h2 + d2, 0.0f, 0.0f}, {0.0f, w2 + d2, 0.0f}, {0.0f, 0.0f, w2 + h2}};
}

constexpr math::Mat3x3f surface_inertia_tensor(Ball const &ball) noexcept {
  auto const r2 = ball.radius * ball.radius;
  return 2.0f / 3.0f * math::Mat3x3f{{r2, 0.0f, 0.0f}, {0.0f, r2, 0.0f}, {0.0f, 0.0f, r2}};
}

constexpr math::Mat3x3f surface_inertia_tensor(Capsule const &capsule) noexcept {
  auto const r = capsule.radius;
  auto const r2 = r * r;
  auto const r3 = r2 * r;
  auto const h = capsule.half_height;
...
constexpr math::Mat3x3f surface_inertia_tensor(Box const &box) noexcept {
  ...
  auto const d2 = d * d;
  return math::Mat3x3f{{
                           d * h * (d2 + h2) + d * w * (d2 + 3.0f * h2) + h * w * (3.0f * d2 + h2),
                           0.0f,
                           0.0f,
                       },
                       {
                           0.0f,
                           w * d * (w2 + d2) + w * h * (w2 + 3.0f * d2) + d * h * (3.0f * w2 + d2),
                           0.0f,
                       },
                       {
                           0.0f,
                           0.0f,
                           w * h * (w2 + h2) + w * d * (w2 + 3.0f * h2) + h * d * (3.0f * w2 + h2),
                       }} /
         (3.0f * (w * h + w * d + h * d));
}
} // namespace physics
