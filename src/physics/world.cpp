        ...
        std::visit(
            [&](auto &&first_object) {
              object_derived_data[0] =
                  derived_data(_intrinsic_state->objects->data(first_object));
            },
            objects.first_specific());
        std::visit(
            [&](auto &&second_object) {
              object_derived_data[1] =
                  derived_data(_intrinsic_state->objects->data(second_object));
            },
...
private:
  std::optional<Narrowphase_result>
  do_narrowphase(Object_pair generic) const noexcept {
    auto result = std::optional<Narrowphase_result>{};
    std::visit(
        [&](auto const specific) {
          result = object_object_contact(
              *_intrinsic_state->objects->data(specific.first),
...
public:
  ...
  void run(Size /*thread_index*/) final {
    for (auto &work_item : _work_items) {
      work_item.constraint->solve_position(work_item.objects,
                                           *_intrinsic_state->objects);
    }
    if (auto const latch = _intrinsic_state->latch) {
      latch->count_down();
    }
  ...
  void run(Size /*thread_index*/) final {
    for (auto const &work_item : _work_items) {
      work_item.constraint->solve_velocity(
          work_item.objects,
          *_intrinsic_state->objects,
          _intrinsic_state->restitution_epsilon);
    }
    if (auto const latch = _intrinsic_state->latch) {
      latch->count_down();
    }
...
public:
  ...
  memory_requirement(World_create_info const &create_info) {
    return Stack_allocator<>::memory_requirement({
        decltype(_objects)::memory_requirement(create_info.max_particles,
  ...
  explicit Impl(World_create_info const &create_info)
    ...
    util::assign_merged(
        ...
        std::tie(_objects,
                 ...
                 _narrowphase_tasks),
        // _objects
        std::tuple{create_info.max_particles,
                   create_info.max_rigid_bodies,
                   create_info.max_static_bodies},
        // _bvh
        std::tuple{create_info.max_aabb_tree_leaf_nodes,
                   create_info.max_aabb_tree_internal_nodes},
        // _neighbor_pairs
        ...
        // _neighbor_groups
        std::tuple{create_info.max_particles + create_info.max_rigid_bodies,
                   create_info.max_neighbor_groups},
        // _awake_neighbor_group_indices
  ...
  Particle create_particle(Particle_create_info const &create_info) {
    ...
    try {
      auto const particle =
          _objects.create_particle(bvh_node,
  ...
  Rigid_body create_rigid_body(Rigid_body_create_info const &create_info) {
    ...
    try {
      auto const rigid_body =
          _objects.create_rigid_body(bvh_node,
                                     create_info.motion_callback,
                                     create_info.position,
                                     create_info.velocity,
  ...
  Static_body create_static_body(Static_body_create_info const &create_info) {
    auto const bvh_node = _bvh.create_leaf(
        ...
        {});
    try {
      auto const static_body =
          _objects.create_static_body(bvh_node,
                                      create_info.position,
                                      create_info.orientation,
                                      create_info.shape,
                                      create_info.material);
...
private:
  ...
  void find_neighbors() {
    ...
    for (auto &pair : _neighbor_pairs) {
      ...
      std::visit(
          [&](auto const object) {
            if constexpr (!std::is_same_v<std::decay_t<decltype(object)>,
                                          Static_body>) {
...
void World::destroy_object(Object generic) {
  std::visit([this](auto const specific) { destroy_object(specific); },
             generic.specific());
}
