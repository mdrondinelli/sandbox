...
Object_position_solve_data position_solve_data(Particle_data const &data) noexcept {
...
Object_position_solve_data position_solve_data(Rigid_body_data const &data) noexcept {
...
Object_position_solve_data position_solve_data(Static_body_data const &data) noexcept {
  auto const transform = Mat3x4f::rigid(data.position(), data.orientation());
  return Object_position_solve_data{
      .transform = transform,
      .inverse_transform = rigid_inverse(transform),
...
void apply_positional_impulse(Particle_data *object_data,
  ...
  object_data->position(object_data->position() + global_impulse * derived_data.inverse_mass);
}
...
void apply_positional_impulse(Rigid_body_data *object_data,
                              ...
                              Vec3f const &global_impulse) noexcept {
  auto const rotation =
      Mat3x3f{{derived_data.transform[0][0], derived_data.transform[0][1], derived_data.transform[0][2]},
              {derived_data.transform[1][0], derived_data.transform[1][1], derived_data.transform[1][2]},
              {derived_data.transform[2][0], derived_data.transform[2][1], derived_data.transform[2][2]}};
  auto const rotated_inverse_inertia_tensor = rotation * derived_data.inverse_inertia_tensor;
  object_data->position(object_data->position() + global_impulse * derived_data.inverse_mass);
  object_data->orientation(object_data->orientation() +
                           0.5f * Quatf{0.0f, rotated_inverse_inertia_tensor * cross(local_position, local_impulse)} *
                               object_data->orientation());
}

void apply_positional_impulse(Static_body_data * /*object_data*/,
                              Object_position_solve_data const & /*derived_data*/,
                              Vec3f const & /*local_position*/,
                              Vec3f const & /*local_impulse*/,
                              Vec3f const & /*global_impulse*/) noexcept {}
...
void Contact::solve_position(Object_pair objects, Object_storage &storage) noexcept {
  std::visit(
      [&](auto const objects) {
        auto const object_data = std::pair{storage.data(objects.first), storage.data(objects.second)};
        auto const object_derived_data = std::array<Object_position_solve_data, 2>{
        ...
        auto const relative_position = contact_positions[0] - contact_positions[1];
        auto const separation = dot(relative_position, normal) + initial_separation;
        // if (separation >= 0.0f) {
        //   return;
        // }
        ...
        auto const generalized_inverse_masses = std::array<float, 2>{
            generalized_inverse_mass(object_derived_data[0].inverse_mass,
                                     object_derived_data[0].inverse_inertia_tensor,
                                     local_positions[0],
                                     local_contact_normals[0]),
            generalized_inverse_mass(object_derived_data[1].inverse_mass,
        ...
        apply_positional_impulse(
            object_data.first, object_derived_data[0], local_positions[0], local_impulses[0], global_impulse);
        apply_positional_impulse(
            object_data.second, object_derived_data[1], local_positions[1], local_impulses[1], -global_impulse);
      },
      objects.specific());
}
...
Object_velocity_solve_data velocity_solve_data(Particle_data const &data) noexcept {
  return {
...
Object_velocity_solve_data velocity_solve_data(Rigid_body_data const &data) noexcept {
...
Object_velocity_solve_data velocity_solve_data(Static_body_data const &data) noexcept {
  auto const rotation = Mat3x3f::rotation(data.orientation());
  return {
      .velocity = Vec3f::zero(),
      .angular_velocity = Vec3f::zero(),
...
void apply_velocity_impulse(Particle_data *object_data,
                            ...
                            Vec3f const & /*local_impulse*/,
                            Vec3f const &global_impulse) noexcept {
  object_data->velocity(object_data->velocity() + derived_data.inverse_mass * global_impulse);
}
...
void apply_velocity_impulse(Rigid_body_data *object_data,
  ...
  object_data->angular_velocity(object_data->angular_velocity() +
                                derived_data.rotation *
                                    (derived_data.inverse_inertia_tensor * cross(local_position, local_impulse)));
}
...
void Contact::solve_velocity(Object_pair objects, Object_storage &storage, float restitution_epsilon) noexcept {
  std::visit(
      [&](auto const objects) {
        auto const object_data = std::pair{storage.data(objects.first), storage.data(objects.second)};
        auto const object_derived_data = std::array<Object_velocity_solve_data, 2>{
        ...
        auto const relative_velocity = (object_derived_data[0].velocity +
                                        cross(object_derived_data[0].angular_velocity, relative_contact_positions[0])) -
                                       (object_derived_data[1].velocity +
                                        cross(object_derived_data[1].angular_velocity, relative_contact_positions[1]));
        ...
        auto const restitution_coefficient = -separating_velocity > restitution_epsilon
                                                 ? 0.5f * (object_derived_data[0].material.restitution_coefficient +
        ...
        auto const normal_impulse_magnitude =
            ...
            (normal_generalized_inverse_masses[0] + normal_generalized_inverse_masses[1]);
        ...
        auto const tangential_velocity = relative_velocity - separating_velocity * normal;
        if (tangential_velocity != Vec3f::zero()) {
          ...
          auto const tangential_generalized_inverse_masses = std::array<float, 2>{
              generalized_inverse_mass(object_derived_data[0].inverse_mass,
                                       object_derived_data[0].inverse_inertia_tensor,
                                       local_positions[0],
                                       -local_tangents[0]),
          ...
          auto const inverse_sum_tangential_generalized_inverse_mass =
              1.0f / (tangential_generalized_inverse_masses[0] + tangential_generalized_inverse_masses[1]);
          auto const static_friction_impulse_magnitude =
              tangential_speed * inverse_sum_tangential_generalized_inverse_mass;
          auto const static_friction_coefficient =
              0.5f * (object_derived_data[0].material.static_friction_coefficient +
                      object_derived_data[1].material.dynamic_friction_coefficient);
          if (static_friction_impulse_magnitude <= static_friction_coefficient * normal_impulse_magnitude) {
            local_impulses[0] -= static_friction_impulse_magnitude * local_tangents[0];
            local_impulses[1] += static_friction_impulse_magnitude * local_tangents[1];
            global_impulse -= static_friction_impulse_magnitude * global_tangent;
          } else {
            auto const dynamic_friction_coefficient =
                0.5f * (object_derived_data[0].material.dynamic_friction_coefficient +
                        object_derived_data[1].material.dynamic_friction_coefficient);
            auto const dynamic_friction_impulse_magnitude = dynamic_friction_coefficient * normal_impulse_magnitude;
            local_impulses[0] -= dynamic_friction_impulse_magnitude * local_tangents[0];
            local_impulses[1] += dynamic_friction_impulse_magnitude * local_tangents[1];
            global_impulse -= dynamic_friction_impulse_magnitude * global_tangent;
          }
        }
